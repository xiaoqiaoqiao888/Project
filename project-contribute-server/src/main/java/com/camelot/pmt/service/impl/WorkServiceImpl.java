...
public class WorkServiceImpl implements WorkService {
    ...
    */
    ...
    public int updateWork(Work work) throws CloneNotSupportedException {
        ...
        int inser1 = logWorkMapper.insertLogWork(logWork);
        //更新阶段状态-待验收
        if (work.getWorkState() != null
                &&
                (work.getWorkState() == Constant.Status.WAIT_CHECK
                        ||
        ...
        if (work.getWorkState() != null
                &&
                (work.getWorkState() == Constant.Status.HAVE_IN_HAND
                        ||
    ...
    public PageInfo<WorkDTO> getWorkCostListByPage(Integer pageNum, Integer pageSize, WorkDTO workDTO) {
        ...
        // 未开始列表（按照创建时间排序）
        List<WorkDTO> noStartCollect = workDTOList.stream()
                .filter(e -> e.getWorkState() == 0).sorted(Comparator.comparing(WorkDTO::getCreateTime))
                .collect(Collectors.toList());
        // 进行中列表（按照确认时间，即修改时间排序）
        List<WorkDTO> haveInHandCollect = workDTOList.stream()
                .filter(e -> e.getWorkState() == 1).sorted(Comparator.comparing(WorkDTO::getUpdateTime))
                .collect(Collectors.toList());
        // 延期进行中（按照确认时间，即修改时间排序）
        List<WorkDTO> delayHaveInHandCollect = workDTOList.stream()
                .filter(e -> e.getWorkState() == 4).sorted(Comparator.comparing(WorkDTO::getUpdateTime))
                .collect(Collectors.toList());
        // 待验收（按照确认时间，即修改时间排序）
        List<WorkDTO> waitForAcceptanceCollect = workDTOList.stream()
                .filter(e -> e.getWorkState() == 2).sorted(Comparator.comparing(WorkDTO::getUpdateTime))
                .collect(Collectors.toList());
        // 已完成（按照确认时间，即修改时间排序）
        List<WorkDTO> completedCollect = workDTOList.stream()
                .filter(e -> e.getWorkState() == 3).sorted(Comparator.comparing(WorkDTO::getUpdateTime))
                .collect(Collectors.toList());
        workDTOList.clear();
    ...
    public void updateStageByid(int id,int flage) throws CloneNotSupportedException {
        if (flage == Constant.Status.HAVE_IN_HAND) {
            ...
            if (flage == Constant.Status.WAIT_CHECK) {
